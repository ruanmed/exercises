...
Theorem rev_app_nil_r: forall X (l : list X),
  rev l ++ [ ] = rev l.
Proof.
  intros X l.
  destruct l as [|x l].
...
Proof.
  intros X l1 l2.
  induction l1 as [| x l1 IHl1].
  - simpl. rewrite -> rev_app_nil_r. reflexivity.
...
Compute (split [(1,true);(2,false);(3,true);(4,true);(5,true);(6,true);(7,false)]).
...
Proof. simpl. reflexivity. Qed.
Example test_hd_error3 : @hd_error bool [] = None.
Proof. simpl. reflexivity. Qed.
(** [] *)
...
Proof. unfold doit3times. simpl. reflexivity.  Qed.

Example test_doit3times': doit3times negb true = false.
Proof. unfold doit3times. reflexivity. Qed.
...
Proof. unfold filter. simpl. reflexivity.  Qed.
...
Proof. unfold countoddmembers'. unfold filter. simpl. reflexivity.  Qed.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun x => andb (evenb x) (ltb 7 x)) l.
...
Proof. simpl. unfold oddb. simpl. reflexivity.  Qed.
...
Proof. unfold fold_length. Check @fold. unfold fold. simpl. reflexivity. Qed.
...
Compute (fold_map (evenb) [1;2;3;4;4]).
...
Proof. unfold plus. unfold three. unfold doit3times. reflexivity. Qed.
